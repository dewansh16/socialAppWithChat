...
const addToGroup = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    // if(!userId)return res.send("empty user")
    // check if the requester is admin

    const added = await Chat.findById(chatId);
    for (let i = 0; i < added.users.length; i++) {
      if (added.users[i] === userID) return res.send("user already present");
    }
    added.users.push(userID);
    added.save();
